...
function DashboardContent() {
  ...
  const { data: wells, loading: wellsLoading, error: wellsError } = useWells()
  const { data: revenue, loading: revenueLoading, error: revenueError } = useRevenue()
  ...
  const stats = {
    totalRevenue: revenue?.reduce((sum: number, r: { amount?: number }) => sum + (r.amount || 0), 0) || 0,
    totalExpenses: 1245800, // Mock data for now
    activeWells: wells?.filter((w: { status: string }) => w.status === "active")?.length || 0,
